...
def check(function, inputs_result_pairs):
    ...
    for (inputs, result) in inputs_result_pairs:
        callStr = prefix+', '.join([str_(i) for i in inputs])+suffix
...
def testProblemOnePartC(ts):
    return tree(tokenize(["children","child","two","one","zero","start","end",";"], s))

print("\n\nProblem #1, part (c)...")
try: (tokenize, tree)
except: print("The tokenize() and/or tree() functions are not defined.")
else:
    check('testProblemOnePartC', [\
        (("zero children",), ('Zero',[])),\
